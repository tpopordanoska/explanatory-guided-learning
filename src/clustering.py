...
def run_kmedoids(points, n_clusters, other_points=None, use_labels="False", rng=0, path=None, method=""):
    ...
    np.random.seed(0)
    initial_medoids = np.random.randint(len(points[:, 0:2]), size=n_clusters)
    ...
    if other_points is not None:
        plt.scatter(other_points[:, 0], other_points[:, 1], c=other_points[:, 3], marker='o', s=100, edgecolor='k')
    else:
        plt.scatter(points[:, 0], points[:, 1], c=points[:, 3], marker='o', s=100, edgecolor='k')
    # Plot the centroids
    ...
    plt.title("K-Medoids")
    if path:
        plt.savefig(path + "\\" + datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + " K-medoids " + method + ".png")
    else:
        plt.show()
    plt.close()
