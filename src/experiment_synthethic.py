...
class Synthetic(Experiment):
    def __init__(self, balanced_db=True, tiny_clusters=True, rng=0):
        ...
        if not balanced_db:
            ...
            # indices, _ = self.split_once(X=X_pos, y=y_pos, proportion=0.6, rng=rng)
            # X_pos, y_pos = X_pos[indices], y_pos[indices]
    ...
    @staticmethod
    def split_once(X, y, proportion, n_splits=1, rng=0):
        """
        ...
        """
        folds = StratifiedShuffleSplit(n_splits=n_splits, test_size=proportion, random_state=rng).split(X, y)
        return list(folds)[0]