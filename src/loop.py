...
def optimal_user(theta, **kwargs):
    ...
    clusters, centroids = run_kmedoids(kmedoids_pd.to_numpy(), no_clusters, rng=rng, path=path,
                                       plots_off=plots_off)
    ...
    run_kmedoids(kmedoids_pd.to_numpy(), no_clusters, other_points=wrong_points, rng=rng, path=path,
                 plots_off=plots_off)
...
class ActiveLearningLoop:
    ...
    def retrain_and_get_acc(self, known_idx, train_idx, test_idx, acc_scores, test_acc_scores):
    ...
    def run(self, method, known_idx, train_idx, test_idx):
        ...
        y_pred, y_pred_test, scores, test_scores = self.retrain_and_get_acc(known_idx, train_idx, test_idx, [], [])
        ...
        for iteration in range(self.max_iter):
            ...
            y_pred, y_pred_test, scores, test_scores = self.retrain_and_get_acc(known_idx, train_idx, test_idx, scores,
                                                                                test_scores)
