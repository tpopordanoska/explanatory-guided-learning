...
def generate_negative(axis, how_many):
    """
    ...
    """
    np.random.seed(0)
    pointsn = np.random.rand(how_many, 2) * (axis-1)
...
def split_data(X, y, proportion, n_splits=1):
    """
    ...
    """
    fold = StratifiedShuffleSplit(n_splits=n_splits, test_size=proportion, random_state=0).split(X, y)
    return list(fold)[0]
